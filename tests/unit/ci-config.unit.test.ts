...
describe('CI and hook configuration', () => {
  ...
  test('test workflow runs critical PR work in parallel and exposes a single pr_status gate', () => {
    ...
    expect(workflow).toContain('name: proxy (ubuntu-latest)');
    expect(workflow).toContain('permissions:\n      contents: read');
    ...
    expect(workflow).toContain('- proxy_linux');
    ...
    expect(workflow).toContain('proxy_linux=${{ needs.proxy_linux.result }}');
    ...
    expect(workflow).toContain('bun run proxy:typecheck');
    expect(workflow).toContain('bun run proxy:build');
    expect(workflow).toContain('bun test tests/e2e/proxy-relay.e2e.test.ts');
